...
def main(): 
    ...
    neigh.fit(X_train_used[:10000], y_train_used[:10000])
    pred_y = neigh.predict(X_test_used[200:500])
    print("pred_y =v", pred_y )
    print("y_test_used[:50]= ",y_test_used[200:500] )
    mae = np.mean(np.abs(pred_y - y_test_used[200:500]))
    accuracy = np.mean(pred_y == y_test_used[200:500])
